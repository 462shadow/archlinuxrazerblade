...
#  if [[ ! -f $HOME/.local/share/gtk-engines/murrine.xml ]]; then
#     
...
#  fi
  ...
  rm $HOME/.local/share/applications/ba-*.desktop 2> /dev/null || true 
  rm $HOME/.local/share/desktop-directories/BlackArch*.directory 2> /dev/null || true

  sudo pacman -S blackmate --overwrite $HOME/.local/share/blackmate --noconfirm

  #Update X.BlackArch.menu
  cp $HOME/.local/share/blackmate/X-BlackArch.menu $HOME/.config/menus/applications-merged/X-BlackArch.menu

  #Copy the directory file BlackArch (Blackarch -> categorie -> tools...)
  cp $HOME/.local/share/blackmate/BlackArch.directory $HOME/.local/share/desktop-directories/

  #Generate the new categorie entry menu
  for u in $( ls --color=auto $HOME/.local/share/blackmate/menu-i/ | sort ); do
  
     c=`echo $u | sed 's/BlackArch-//' | sed 's/\.png//'`;
        cat $HOME/.local/share/blackmate/dfdir | sed 's/^Name=.*/Name='$c'/' | 
	sed 's/^Icon=.*/Icon=BlackArch-'$c'/' > $HOME/.local/share/desktop-directories/BlackArch-$c.directory 
  ...
  if [[ -f $HOME/.config/xfce4/xfconf/xfce-perchannel-xml/xsettings.xml ]]; then
    ...
    thic=`cat $HOME/.config/xfce4/xfconf/xfce-perchannel-xml/xsettings.xml | grep IconThemeName | 
  ...
  rm $HOME/.local/share/icons/$thic/icon-theme.cache 2> /dev/null || true
  
  #Copy the extra icons into the icons theme
  cp $HOME/.local/share/blackmate/menu-i/* $HOME/.local/share/icons/$thic/32x32/apps/ 2> /dev/null || true
  
  #Set the correct chmod
  chmod 755 $HOME/.local/share/icons/$thic/32x32/apps/ -R 2> /dev/null || true
  ...
  if [[ -d "$HOME/.local/share/blackmate/tmp" ]]; then
     rm -rf $HOME/.local/share/blackmate/tmp 2> /dev/null || true
  fi
  
  #Create temporary directory to store the tools list
  mkdir $HOME/.local/share/blackmate/tmp 2> /dev/null || true

  wget -q -P $HOME/.local/share/blackmate/ https://mirror.yandex.ru/mirrors/blackarch/blackarch/os/x86_64/blackarch.db.tar.gz 
  tar -zxf $HOME/.local/share/blackmate/blackarch.db.tar.gz -C $HOME/.local/share/blackmate/tmp
  ...
  for u in $( ls --color=auto $HOME/.local/share/blackmate/tmp/ | sort ); do

	  #Subcategorie
	  subc=`cat $HOME/.local/share/blackmate/tmp/$u/desc | sed 's/blackarch//' | 
	   ...
	   #Name of the tool
	   tname=`cat $HOME/.local/share/blackmate/tmp/$u/desc | sed 's/blackarch//' | 
  	...
  	for i in $tname; do
	    ...
	    cat $HOME/.local/share/blackmate/dfdesk | sed 's/^Name=.*/Name='$i'/' |
	    #Set the bash command to execute
	    sed 's/^Exec=.*/Exec='$terminal' -e "sudo \/usr\/bin\/'$i';bash"/' |
	    #Set the categorie to the launcher && Set the name file to ba-`toolsname`.desktop 
	    sed 's/Categories=.*/Categories='$namecat';/' > $HOME/.local/share/blackmate/ba-$i.desktop
  ...
  mv $HOME/.local/share/blackmate/ba-*.desktop $HOME/.local/share/applications 2> /dev/null || true

  #Delete tmp directory
  rm -rf $HOME/.local/share/blackmate/tmp/ 2> /dev/null || true
  rm $HOME/.local/share/blackmate/blackarch.db.tar.gz 2> /dev/null || true
